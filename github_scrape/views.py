...
def index(request, language=None):
    if language is None:
        ...
        users = GithubUser.objects.filter(language__isnull=True)
    else:
        users = GithubUser.objects.filter(language__iexact=language)
    users = users.order_by('-public_repo_count')
    languages = GithubUser.objects.values_list('language').exclude(language__isnull=True).distinct().order_by('language')
